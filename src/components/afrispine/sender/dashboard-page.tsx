...
export function DashboardPage() {
  ...
  const recentTransfers = [
    { id: 'TXN-001', recipient: 'Jane Wanjiku', amount: '$100.00', receiveAmount: 'KES 15,378', status: 'delivered', date: '28 Jun 2025', corridor: 'US \u2192 KE' },
    { id: 'TXN-002', recipient: 'Emeka Okafor', amount: '$250.00', receiveAmount: 'NGN 391,250', status: 'processing', date: '29 Jun 2025', corridor: 'US \u2192 NG' },
    { id: 'TXN-003', recipient: 'Kwame Asante', amount: '$75.00', receiveAmount: 'GHS 1,149', status: 'delivered', date: '25 Jun 2025', corridor: 'US \u2192 GH' },
    { id: 'TXN-004', recipient: 'Amina Hassan', amount: '$50.00', receiveAmount: 'KES 7,689', status: 'pending', date: '30 Jun 2025', corridor: 'US \u2192 KE' },
    { id: 'TXN-005', recipient: 'Fatou Diallo', amount: '$200.00', receiveAmount: 'GHS 3,064', status: 'delivered', date: '24 Jun 2025', corridor: 'US \u2192 GH' },
  ];
      ...
      <Card className="border-emerald-100 bg-gradient-to-r from-emerald-50/80 to-white">
        <CardContent className="pt-5 pb-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-sm font-semibold text-emerald-700">{`${USD}${monthSent.toLocaleString()} / ${USD}${monthGoal.toLocaleString()}`}</p>
          </div>
          <Progress value={goalPct} className="h-2.5 [&>div]:bg-emerald-500" />
          <p className="mt-1.5 text-xs text-muted-foreground">
            {goalPct >= 100
              ? `Target reached! You saved an estimated ${USD}${(monthSent * 0.055).toFixed(0)} vs traditional providers.`
              : `${USD}${monthGoal - monthSent} more to reach your target. Save up to ${((monthGoal - monthSent) * 0.055).toFixed(0)} in fees with AfriSpine.`}
          </p>
        </CardContent>
      </Card>
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm text-muted-foreground truncate">
                  {hasWealthAccount
                    ...
                    : `From ${USD}10. NSE, NGX, JSE and more.`}
                </p>
              </div>
            </div>
